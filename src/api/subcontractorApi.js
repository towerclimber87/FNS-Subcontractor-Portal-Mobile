...
export function sitePagePath(page, site) {
  ...
  switch (page?.key) {
    case 'site_daily_tracker':
      return withSite('/subcontractor/site_task_tracker');
    case 'daily_reports':
    ...
    default:
      return withSite(page?.path || '/subcontractor/site_task_tracker');
  }
}
...
// Backward-compatible aliases. Some screens/builds used the older DailySite name
// while the current screen uses SiteDaily. Keep both names exported so stale native
// bundles or copied screens do not crash with "... is not a function".
export const loadSubcontractorDailySiteTracker = loadSubcontractorSiteDailyTracker;
export const updateSubcontractorDailySiteTrackerRecord = updateSubcontractorSiteDailyTrackerRecord;
export const uploadSubcontractorDailySiteTrackerPhoto = uploadSubcontractorSiteDailyTrackerPhoto;
